...
{
    internal partial class Dispute : RequestRead
    {
        protected override string RequestId => "disputes";

        protected override Dictionary<string, object> RequestParameters { get; }

        public Dispute(int _cdid = 0, int _cid = 0, int _uid = 0, int _page = 0, int _perpage = 0)
        {
            RequestParameters = new Dictionary<string, object>();
            if (_cdid > 0)
            {
                RequestParameters.Add("_cdid", _cdid);
            }
            if (_cid > 0)
            {
